 ...
 */
public class FractalienOrthoGlRenderer implements GLSurfaceView.Renderer {

	/**
	 * Constructor
	 * @param context
	 */
	public FractalienOrthoGlRenderer(Context context) {
	...
	private void launchThreads() {
		...
		this.fractal.setScale((float)(this.height)/(float)(this.width));
		this.killThreads = false;

		// go forth and create fractal bitmaps
		this.produceFractalThread = new Thread(new Runnable() {

			private int dim = FractalienOrthoGlRenderer.this.kMaxDim;

			@Override
			public void run() {
				while (!FractalienOrthoGlRenderer.this.killThreads) {
					if (!FractalienOrthoGlRenderer.this.fractalProduced) {
						synchronized (FractalienOrthoGlRenderer.this.lock) {
							...
							if (this.dim > FractalienOrthoGlRenderer.this.kMaxDim) {
								FractalFactory.makeItCool(FractalienOrthoGlRenderer.this.fractal, true);
								this.dim = 1;
								FractalienOrthoGlRenderer.this.fractal.setDims(this.dim, this.dim);
							}
							...
							FractalFactory.generateFullFractal(FractalienOrthoGlRenderer.this.fractal, this.dim);
							FractalienOrthoGlRenderer.this.fractalProduced = true;
							Log.d("bob", "produced");

							// Notify waiting threads that we have a new bitmap
							FractalienOrthoGlRenderer.this.lock.notifyAll();
					...
					if (FractalienOrthoGlRenderer.this.fractalProduced) {
						synchronized (FractalienOrthoGlRenderer.this.lock) {
							try {
								...
								FractalienOrthoGlRenderer.this.lock.wait();
							} catch (final InterruptedException e) {
...
//			this.currentAngle -= 360.0f;
	...
	public void onSurfaceChanged(GL10 glUnused, int w, int h) {
		...
		perspective.ortho(-0.5f, 0.5f, -0.5f, 0.5f, 1.f, 10.f);
